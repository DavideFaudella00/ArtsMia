...
public class ArtsmiaDAO {
	...
	public int getPeso(ArtObject a1, ArtObject a2) {
		...
		String sql = "SELECT COUNT(*) as peso " + "FROM exhibition_objects e1, exhibition_objects e2 "
	...
	public List<Adiacenza> getAdiacenze(Map<Integer, ArtObject> idMap) {
		String sql = "SELECT e1.object_id as e1, e2.object_id as e2, COUNT(*) AS peso "
				+ "FROM exhibition_objects e1, exhibition_objects e2 " + "WHERE e1.exhibition_id = e2.exhibition_id "
				+ "AND e1.object_id > e2.object_id " + "GROUP BY e1.object_id, e2.object_id";
		Connection conn = DBConnect.getConnection();
		...
		try {
			...
			while (res.next()) {
				result.add(new Adiacenza(idMap.get(res.getInt("e1")), idMap.get(res.getInt("e2")), res.getInt("peso")));
			}
			conn.close();
			return result;
